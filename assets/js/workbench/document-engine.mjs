...
function renderSection(section) {
  ...
  return `<section class="document-content-section">
    ...
    ${items.length ? renderList(items, section) : ""}
  </section>`;
}
...
function renderCell(cell, column) {
  ...
  const body = descriptor.state
    ? `<span class="document-state${descriptor.muted ? " is-muted" : ""}">${escapeDocumentHtml(value)}</span>`
    : descriptor.strong
      ? `<strong>${escapeDocumentHtml(value)}</strong>`
      : escapeDocumentHtml(value);
  return `<td${classes ? ` class="${classes}"` : ""}>${body}${subtext ? `<small>${escapeDocumentHtml(subtext)}</small>` : ""}</td>`;
}
...
function renderList(items, section) {
  ...
  const cls = section.columnar ? ' class="is-columnar"' : "";
  return `<${tag}${cls}>${items.map((item) => `<li>${escapeDocumentHtml(item)}</li>`).join("")}</${tag}>`;
}
function multiline(value) { return escapeDocumentHtml(value).replace(/\r?\n/g, "<br>"); }
function hasValue(item) { return Boolean(item && text(item.value)); }
function classToken(value) { return text(value).toLowerCase().replace(/[^a-z0-9-]+/g, "-").replace(/^-|-$/g, "") || "document"; }
